...
featr_dir = os.path.join(home_dir, ".featr")
log_dir = os.path.join(featr_dir, 'logs')
log_path = os.path.join(log_dir, LOG_FILE)
prof_dir = os.path.join(featr_dir, 'profiles')
...
def _check_dirs():
    """필요한 디렉토리 체크."""
    if not os.path.isdir(featr_dir):
        make_dir(featr_dir, False)
